...
sub btnOpenClick(btn as unsigned integer,parm as unsigned integer)
    ...
    if (strlen(strLine)>0) then
        var f = TextFile.Open_READ(strline)
        if (f<>0) then
            CursorPos = 0
            ScrollY = 0
            ScrollX = 0
            ...
            TextSize = TextFile.FSize()
            if (TextSize>0) then
                TextFile.Read(TextBuffer,TextSize)
            end if
            TextFile.Close()
        else
...
sub btnSaveClick(btn as unsigned integer,parm as unsigned integer)
    ...
    if (strlen(strLine)>0 and TextSize>0) then
        var f = TextFile.Open_Create(strline)
        TextFile.Write(TextBuffer,TextSize)
        TextFile.Close()
    end if
	Thread_Callback_End()
end sub          
...
sub CheckScroll()
    ...
    if (CursorX+ScrollX<0) then
        ScrollX =0-CursorX
    end if
    if (CursorX+ScrollX>MaxX-1) then
        ScrollX = (MaxX-CursorX)-1
    end if
end sub
