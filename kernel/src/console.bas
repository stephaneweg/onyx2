...
sub ConsoleDrawTitle()
    ...
    memset16(cptr(unsigned short ptr,CONSOLE_MEM),(1* 16 +15) shl 8,consoleWidth)
    var px = ConsoleCursorX
    var py = consoleCursorY
    consoleCursorX          = 0
    consoleCursorY          = 0
...
sub ConsoleScroll()
    dim dst as unsigned integer = cuint(CONSOLE_MEM) + (consoleWidth*2)
    memcpy16(cptr(unsigned short ptr,dst),cptr(unsigned short ptr,dst+(consoleWidth*2)),consoleWidth*(consoleHeight-2))
    memset16(cptr(unsigned short ptr,dst+consoleWidth*(consoleHeight-2)*2),(consoleBackground* 16 +consoleForeground) shl 8,consoleWidth)
...
sub ConsoleClear()
    memset16(cptr(unsigned short ptr,cuint(CONSOLE_MEM)+(ConsoleWidth*2)),(consoleBackground* 16 +consoleForeground) shl 8,consoleWidth*(consoleHeight-1))
    consoleCursorX = 0
    consoleCursorY = 1
    consoleLineOffset = consoleCursorY*ConsoleWidth
    ConsoleUpdateCursor()
