...
function IPCEndPoint.CreateAnonymous(th as thread ptr,entry as unsigned integer,sync as unsigned integer) as IPCEndPoint ptr
    'if name is already used return 0
    return IPCEndPoint.CreateID(IPCEndPoint.NewID(),th,entry,sync)
end function
...
function IPCEndPoint.CreateID(id as unsigned integer,th as thread ptr,entry as unsigned integer,sync as unsigned integer) as IPCEndPoint ptr
    if (id>=lbound(IRQ_HANDLERS) and id<=ubound(IRQ_HANDLERS)) then
        if (IRQ_HANDLERS(id)<>0)  then 
            ConsoleWrite(@"ID 0x"):ConsoleWriteNumber(id,16):ConsoleWriteLine(@" already used by irq handler")
            return 0 'cannot ipc end point
        end if
    end if
    
    if (IPCEndPoint.FindBYId(id)<>0) then
        ConsoleWrite(@"ID 0x"):ConsoleWriteNumber(id,16):ConsoleWriteLine(@" already used")
...
sub IPC_THREAD_TERMINATED(t as unsigned integer)
